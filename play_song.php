<?php
	...
	$query = "SELECT * FROM Plays WHERE song_id='$song_id' AND user_id='$user_id';";
	$result = mysql_query($query);

	# if so, update, otherwise insert a new row into Plays
	if($row = mysql_fetch_array($result)) {
		$update = "UPDATE Plays SET plays = plays + 1 WHERE song_id='$song_id' AND user_id='$user_id';";
		mysql_query($update);
	} else {
		$insert = "INSERT INTO Plays(song_id, user_id, plays) VALUES('$song_id','$user_id',1);";
		mysql_query($insert);
	}

	# and add an event object
	$event = "INSERT INTO Events(song_id, user_id, etype) VALUES('$song_id', '$user_id', 'play')";
	mysql_query($event);
